  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto text-[#0F172A]">
      ...
      {isLoading ? (
        ...
        <Card className="p-16 text-center text-slate-400 glass-bento">
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-500 pt-1">
                    <span className="flex items-center gap-1.5 font-medium">
                      ...
                      Candidate: <strong className="text-slate-800 ml-1">{app.studentName}</strong> ({app.studentEmail})
                    </span>
                  </div>
                </div>

                {/* Score & Action Controls */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 border-t md:border-t-0 md:border-l border-slate-200/80 pt-4 md:pt-0 md:pl-6 w-full md:w-auto justify-between md:justify-end">
                  <div className="text-left sm:text-right">
                    <p className="text-[10px] uppercase font-mono font-medium text-slate-500 tracking-wider">
                      Competency Score
                    </p>
                    <p className="text-2xl sm:text-3xl font-extrabold text-blue-600 font-mono">{app.matchPercent}%</p>
                  </div>

                  {isAdmin && (
                    <div className="space-y-1.5">
                      <label className="block text-[10px] uppercase font-mono font-medium text-slate-500 tracking-wider">
                        Update Pipeline Stage
                      </label>
                      <select
                        value={app.status}
                        onChange={(e) => handleStatusChange(app.id, e.target.value as ApplicationStatus)}
                        className="px-3 py-2 glass-input rounded-xl text-xs font-medium text-slate-900 focus:outline-none cursor-pointer"
                      >
                        <option value="Submitted" className="bg-white text-slate-900">
                          Submitted
                        </option>
                        <option value="Under Review" className="bg-white text-slate-900">
                          Under Review
                        </option>
                        <option value="Interviewing" className="bg-white text-slate-900">
                          Interviewing
                        </option>
                        <option value="Accepted" className="bg-white text-slate-900">
                          Accepted
                        </option>
                        <option value="Rejected" className="bg-white text-slate-900">
                          Rejected
                        </option>
                      </select>
                    </div>
