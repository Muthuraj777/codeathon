    ...
    <Card className="glass-bento overflow-hidden relative">
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-zinc-800/80">
        <div className="space-y-1">
          <CardTitle className="flex items-center gap-2.5 text-white text-base font-semibold">
      ...
      <CardContent className="space-y-4 pt-5">
        {topSkillGaps.length === 0 ? (
          <div className="py-16 text-center text-zinc-500 text-xs flex flex-col items-center justify-center space-y-2">
            <BarChart2 className="w-9 h-9 text-zinc-700 mb-1" />
            <p className="font-medium text-zinc-400">No skill gap deficiencies recorded yet.</p>
            <p className="text-zinc-500 max-w-sm">Run a skill gap matrix calculation or configure target job role criteria.</p>
          </div>
        ) : (
          topSkillGaps.map((skill) => (
            <div key={skill.skillName} className="space-y-2 group p-2 rounded-xl hover:bg-zinc-800/30 transition">
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-zinc-100 group-hover:text-indigo-300 transition-colors">
                    {skill.skillName}
                  </span>
                  <span className="text-[10px] text-zinc-400 font-mono px-2 py-0.5 rounded-full bg-zinc-950/80 border border-zinc-800">
                    {skill.category}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[11px] font-medium text-zinc-400">{skill.gapCount} Candidates Gap</span>
                  <span className="font-mono text-xs font-semibold text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-lg">
                    {skill.percentage}%
                  </span>
                </div>
              </div>
              ...
              <div className="w-full h-2.5 bg-zinc-950 rounded-full overflow-hidden p-0.5 border border-zinc-800/80">
                <div
                  className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-400 rounded-full transition-all duration-1000 ease-out shadow-xs shadow-indigo-500/30"
