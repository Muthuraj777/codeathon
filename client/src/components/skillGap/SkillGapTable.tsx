...
export const SkillGapTable: React.FC<SkillGapTableProps> = ({ items }) => {
  return (
    <Card className="border-slate-800/80 bg-slate-900/70 backdrop-blur-xl shadow-xl">
      ...
      <CardContent className="p-0 sm:p-6">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800/80 text-[11px] font-mono-code font-bold text-slate-400 uppercase tracking-wider bg-slate-950/80">
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs sm:text-sm">
              {items.map((item) => (
                <tr key={item.skillId} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-4 font-bold text-slate-100">{item.skillName}</td>
                  <td className="p-4 font-semibold text-slate-300">
                    <span className="inline-flex items-center justify-center w-7 h-7 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 font-mono-code text-xs shadow-inner">
                      {item.currentLevel}
                    </span>
                    <span className="text-xs text-slate-500 ml-1">/ 5</span>
                  </td>
                  <td className="p-4 font-semibold text-slate-300">
                    <span className="inline-flex items-center justify-center w-7 h-7 bg-indigo-950/70 border border-indigo-800/60 text-indigo-300 rounded-lg font-mono-code text-xs shadow-inner">
                      {item.requiredLevel}
                    </span>
                    <span className="text-xs text-slate-500 ml-1">/ 5</span>
                  </td>
                  <td className="p-4 font-mono-code font-bold">
                    {item.gap > 0 ? (
                      <span className="text-rose-300 bg-rose-950/60 border border-rose-800/80 px-2.5 py-1 rounded-lg text-xs">
                        -{item.gap}
                      </span>
                    ) : (
                      <span className="text-emerald-300 bg-emerald-950/60 border border-emerald-800/80 px-2.5 py-1 rounded-lg text-xs">
                        0
                      </span>
                    )}
                  </td>
