...
export const ApplyJobModal: React.FC<ApplyJobModalProps> = ({ isOpen, onClose, job, matchPercent }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900/95 rounded-3xl border border-slate-800 shadow-2xl shadow-indigo-950/50 w-full max-w-lg overflow-hidden backdrop-blur-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-6 bg-slate-950/60 border-b border-slate-800/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Review application parameters before sending to recruiters</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800/80 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h4 className="font-bold text-slate-100 text-base leading-snug">{job.title}</h4>
                <p className="text-xs text-indigo-400 flex items-center gap-1 mt-1 font-semibold">
                  <Building2 className="w-3.5 h-3.5 text-slate-400" />
                  {job.company}
                </p>
              </div>
              <Badge variant="primary" className="text-xs shrink-0 font-mono-code">
                {matchPercent}% Match
              </Badge>
            </div>
          </div>
          ...
          <div className="space-y-2 border-t border-slate-800/80 pt-4">
            <h5 className="text-[11px] font-mono-code font-bold uppercase text-slate-400 tracking-wider">
            ...
            <div className="p-4 bg-indigo-950/20 rounded-2xl border border-indigo-800/40 space-y-1">
              ...
              <Badge variant="success" className="capitalize text-[10px] mt-1 font-mono-code">
                {user?.role || 'Student'}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 p-3.5 bg-emerald-950/40 rounded-xl border border-emerald-800/60 text-xs text-emerald-300 font-medium">
            ...
            <span>
              Your match score of <strong className="text-emerald-200 font-mono-code">{matchPercent}%</strong> will be attached to your submission.
            </span>
          </div>
